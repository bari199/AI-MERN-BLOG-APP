...
import { LuGalleryVertical, LuLoaderCircle, LuPlus } from "react-icons/lu";
import { useNavigate } from "react-router-dom";
import Tabs from "../../../components/Loader/Tab";
...
const BlogPost = () => {
  ...
  const deletePost = async (postId) => {
    try {
      //const postId = openDeleteAlert.data;
      ...
      // remove from UI
      setBlogPostList((prev) => prev.filter((p) => p._id !== postId));
  ...
  return (
    <DashboardLayout activeMenu="Blog post">
      <div className="w-auto sm:max-w-[900px] mx-auto">
        ...
        <div className="flex items-center justify-center mb-8 ">
          <button
            className="flex items-center gap-3 text-sm text-white font-medium bg-black px-7 py-2 rounded-full text-nowrap hover:scale-105 transition-all cursor-pointer"
            disabled={isLoading}
          ...
          >
            {isLoading ? (
              ...
              <LuGalleryVertical className="text-lg" />
            )}
